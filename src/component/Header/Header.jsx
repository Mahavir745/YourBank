...
const Header = ({isLogin}) => {
  ...
  return (
    <div className='size-full h-40 bg-gray-900 flex items-center justify-between sm:justify-around p-2 text-white sticky top-0 z-50'>
      <div className=''>
       <h1 className='text-6xl font-bold flex'><span className='text-sky-300'>Your</span><span>Bank</span><BsBank className='pl-2 '/></h1>
      </div>
      {foundWidth < 480 ?
        <div className='relative'>
          {!menuState ? <BiMenu className='text-xl' onClick={HandleToggle} /> : <RxCross1 className='text-xl' onClick={HandleToggle} />}
          {menuState &&
            <ul className='flex gap-2 flex-col absolute bg-slate-300 p-2 right-[0px] w-[140px] rounded-md' ref={menuRef}>
              ...
              <NavLink className={({ isActive }) => `${isActive ? "text-gray-700" : "text-white"}`} to="/createAccount">
                <li className='font-bold hover:bg-slate-400 pl-4 pr-4 pt-2 pb-2 rounded-md'>Create Account/Login</li>
              </NavLink>          
